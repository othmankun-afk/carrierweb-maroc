...
export default async function SolutionsListingView({ locale }: { locale: Locale }) {
  const dict = getDict(locale);
  const t = dict.listing.solutions;
      ...
      <main id="main-content" className="bg-cw-black min-h-screen pt-[110px] pb-20">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {solutions.map((sol) => (
              ...
              >
                <div>
                  <div className="w-12 h-12 rounded-xl bg-cw-red/10 border border-cw-red/20 flex items-center justify-center text-cw-red-light mb-5 group-hover:bg-cw-red group-hover:text-white transition-all">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
          <div className="bg-gradient-to-br from-cw-red/10 to-cw-surface border border-cw-red/20 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            ...
            <Link href={lp("/secteurs")} className="cta-outline text-xs px-5 py-3 flex-shrink-0">
              {t.crossCta}
